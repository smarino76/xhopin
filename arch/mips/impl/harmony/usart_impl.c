...
{
    /* Harmony già inizializza i moduli all'avvio */
}
 ...
 *
 * 1. NUMERO DI ISTANZE:
 *    Il PIC32MZ ha 6 moduli UART (UART1-UART6), contro i 4 del PIC32MX.
 *    Questo richiede un array di callback di dimensione 6 invece di 4.
 *
 * 2. FUNZIONI IDENTICHE:
 *    Le API Harmony per UART sono identiche tra PIC32MX e PIC32MZ:
 *    - UARTx_Initialize()
 *    - UARTx_SerialSetup()
 *    - UARTx_Read()
 *    - UARTx_Write()
 *    - UARTx_ReadCallbackRegister()
 *    - UARTx_WriteCallbackRegister()
 *
 * 3. FREQUENZA:
 *    La funzione UARTx_FrequencyGet() restituisce la frequenza corretta
 *    per ciascun dispositivo. Per PIC32MZ con CPU a 200MHz, restituirà
 *    100000000UL (PBCLK2 = CPU_HZ / 2).
 *
 * ============================================================================
 */
...
/* Include solo le istanze abilitate in CONFIG_UART_MASK */
    ...
    #include "peripheral/uart/plib_uart4.h"
#endif
#if (CONFIG_UART_MASK & UART5_MASK)
    #include "peripheral/uart/plib_uart5.h"
#endif
#if (CONFIG_UART_MASK & UART6_MASK)
    #include "peripheral/uart/plib_uart6.h"
#endif

/* Storage per callback (una per istanza - PIC32MZ ha 6 UART) */
#define UART_MAX_INSTANCES 6
static arch_uart_cb_t uart_callbacks[UART_MAX_INSTANCES];
static uintptr_t uart_contexts[UART_MAX_INSTANCES];

/* Wrapper generico: riceve l'ID dell'istanza come contesto */
static void uart_generic_cb(uintptr_t context)
{
    uint8_t id = (uint8_t)context;
    if (id < UART_MAX_INSTANCES && uart_callbacks[id])
        uart_callbacks[id](uart_contexts[id]);
}
...
#if (CONFIG_UART_MASK & UART5_MASK)
        case 4:
            if (!HAS_UART(UART5_MASK)) return -1;
            if (!UART5_SerialSetup(&setup, UART5_FrequencyGet()))
                return -1;
            return 0;
#endif
#if (CONFIG_UART_MASK & UART6_MASK)
        case 5:
            if (!HAS_UART(UART6_MASK)) return -1;
            if (!UART6_SerialSetup(&setup, UART6_FrequencyGet()))
                return -1;
            return 0;
#endif
        default:
...
#if (CONFIG_UART_MASK & UART5_MASK)
        case 4:
            return UART5_Read(buffer, size) ? 0 : -1;
#endif
#if (CONFIG_UART_MASK & UART6_MASK)
        case 5:
            return UART6_Read(buffer, size) ? 0 : -1;
#endif
        default:
            return -1;
    }
...
#if (CONFIG_UART_MASK & UART5_MASK)
        case 4:
            return UART5_Write((void*)buffer, size) ? 0 : -1;
#endif
#if (CONFIG_UART_MASK & UART6_MASK)
        case 5:
            return UART6_Write((void*)buffer, size) ? 0 : -1;
#endif
        default:
            return -1;
    }
...
int arch_uart_impl_set_callback(uint8_t id, arch_uart_cb_t cb, uintptr_t ctx)
{
    if (id >= UART_MAX_INSTANCES) return -1;
...
#if (CONFIG_UART_MASK & UART5_MASK)
        case 4:
            UART5_ReadCallbackRegister(uart_generic_cb, (uintptr_t)id);
            return 0;
#endif
#if (CONFIG_UART_MASK & UART6_MASK)
        case 5:
            UART6_ReadCallbackRegister(uart_generic_cb, (uintptr_t)id);
            return 0;
#endif
        default:
            return -1;
